...
namespace DBCompiler_sql
{
    ...
    class DbCompiler
    {
        ...
        static void BuildTable(DbTable table, StringBuilder code)
        {
            ...
            code.AppendLine("\tif release && this.m_loaded {");
            foreach (var c in table.Columns)
            {
                if (!c.InUse)
                {
            ...
            if (table.SingleRow)
            {
                ...
                code.AppendLine("\t\terr = this.Save(false)");
                code.AppendLine("\t\tif err != nil {");
            ...
            {
                //save
				code.AppendLine("func (this *" + table_name + ") Save(quick bool) (err error) {");
                code.AppendLine("\tif this.m_row==nil{");
            ...
            {
                //fetch rows                
                code.AppendLine("func (this *" + table_name + ") fetch_rows(rows " + row_map_name + ") (r " + row_map_name + ") {");
                ...
                //fetch_new_rows
				code.AppendLine("func (this *" + table_name + ") fetch_new_rows() (new_rows " + row_map_name + ") {");
				...
				code.AppendLine("\tremoved_rows := this.fetch_rows(this.m_removed_rows)");
                ...
                code.AppendLine("\t}");
                code.AppendLine("\tthis.m_removed_rows = make(" + row_map_name + ")");
				code.AppendLine("\trows := this.fetch_rows(this.m_rows)");
                code.AppendLine("\tthis.save_rows(rows, quick)");
				code.AppendLine("\tnew_rows := this.fetch_new_rows()");
        ...
        public static void Compile(string lang, string def_file, string proto_pfefix_file, string proto_file, string code_prefix_file, string code_file)
        {
            ...
            code.AppendLine("\t\treturn");
            code.AppendLine("\t}");
            code.AppendLine("\terr = this.Save(true)");
            code.AppendLine("\tif err != nil {");
            ...
            //dbc save
            code.AppendLine("func (this *DBC)Save(quick bool)(err error){");
            foreach (var t in def.Tables)
            {
                string member_name = t.SingleRow ? t.TableName : t.TableName + "s";
				code.AppendLine("\terr = this." + member_name + ".Save(quick)");
                code.AppendLine("\tif err != nil {");
